...
class PortalUser(http.Controller):
    ...
    def appointment_form(self, **kw):
        ...
        # dates = request.env["x_allocated_technician"].sudo().search([])
        dates = []
        values.update({
            'services': services,
            'dates': dates
    ...
    def get_area(self, **kw):
        ...
        print("\n\n\nallocated_technician_line_64c6d>>>", allocated_technician_line_64c6d)
    ...
    def get_available_dates(self, **kw):
        ...
        print("\n\n\nallocated_technician_line_64c6d2222222222>>>", allocated_technician_line_64c6d)
        allocated_technician = allocated_technician_line_64c6d.mapped('x_allocated_technician_id')
        print("\n\n\nallocated_technician3333333333>>>", allocated_technician)
        ...
        print("\n\navailable_dates_list>>", available_dates_list)
        option_value = '[%s]' % ', '.join(map(str, available_dates_list))
        print("\n\noption_value>>>", option_value)
        return option_value

    '''
           Added Function to set date
        '''

    @http.route('/get_date', type='http', auth='public', website=True)
    def get_date(self, **kw):
        print("\n\n\n\nkw>>>", kw)
        allocated_technician = request.env['x_allocated_technician'].sudo().search(
            [('id', '=', int(kw.get('service_date')))])
        print("\n\nallocated_technician>>>", allocated_technician)
        x_studio_date = allocated_technician.x_studio_date.strftime("%m/%d/%Y")
        print("\n\nx_studio_date>>>", x_studio_date)
        print("\n\ntype(x_studio_date)>>>", type(x_studio_date))
        return x_studio_date